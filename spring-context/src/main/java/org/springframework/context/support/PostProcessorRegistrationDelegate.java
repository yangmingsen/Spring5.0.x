...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				...
				else { // 否则则将其当做普通的BeanFactoryPostProcessor处理,直接加入regularPostProcessors集合,以备后续处理
		...
		else {  // 2、如果不是BeanDefinitionRegistry的实例,那么直接调用其回调函数即可-->postProcessBeanFactory
		...
		for (String ppName : postProcessorNames) {
			...
			else if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) { //// 加入到PriorityOrdered有序处理器集合
				priorityOrderedPostProcessors.add(beanFactory.getBean(ppName, BeanFactoryPostProcessor.class));
			}
			else if (beanFactory.isTypeMatch(ppName, Ordered.class)) { // 加入到Ordered有序处理器集合
	...
	public static void registerBeanPostProcessors(
		...
		int beanProcessorTargetCount = beanFactory.getBeanPostProcessorCount() + 1 + postProcessorNames.length; //#issue 没明白？
